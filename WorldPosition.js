...
class WorldPosition
{
	...
	static midPointRoomPosition(fromPos, toPos, lerpAmount)
	{
		let fromWpos = fromPos.toWorldPosition();
		let toWpos = toPos.toWorldPosition();
		let midX = Math.floor(Math.lerp(fromWpos.x, toWpos.x, lerpAmount));
		let midY = Math.floor(Math.lerp(fromWpos.y, toWpos.y, lerpAmount));
		let midWpos = new WorldPosition(midX, midY);
		return midWpos.toRoomPosition();
	}
