...
class Job_Commando extends Job
{
    constructor (...args)
    {
        super(...args);

        //console.log('Job_Commando.constructor - executing');

        this.jobType = 'commando';
    ...
    {
        // let commandoFlag = Game.flags['commando_' + this.roomName] || Game.flags['commando'];
        // // if (!commandoFlag || commandoFlag.pos.roomName != this.roomName)
        // //     return null;
        ...
        if (!task)
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredSpawn - ' + this.roomName + ' - TASK not available to ' + spawn.room.name );
            return null;
        }
        ...
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredSpawn - ' + this.roomName + ' - PARTS not available to ' + spawn.room.name );
            return null;
        }
        ...
        if (!boosts)
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredSpawn - ' + this.roomName + ' - BOOSTS not available to ' + spawn.room.name );
            return null;
        }

        //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredSpawn - ' + this.roomName + ' - requesting spawn in ' + spawn.room.name );
    ...
    getTask(creep, spawn)
    {
        ...
        let desiredCount = towerCount;
        if (commandoFlag)
            desiredCount = commandoFlag.color % 9;//
        ...
        //console.log('XXXXXXXXXXXXXXXX Job_Commando.getTask - ' + this.roomName + ' - task available to ' + searchingRoom.name );

        return { utility: 1.0, jobId: this.id, jobType: this.jobType, name: 'commando', program: 'task_commando', data: { r: this.roomName }};
    }
    ...
    getDesiredBoosts(spawn, partList)
    {
        ...
        //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + JSON.stringify(partCounts))
        ...
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + this.roomName + ' - XZHO2 desired: ' + LAB_BOOST_MINERAL * moveCount + ', available: ' + Room.getStoredResourceAmount(spawn.room.name, 'XZHO2'));
            return null;
        }
        
        if (Room.getStoredResourceAmount(spawn.room.name, 'XGHO2') < LAB_BOOST_MINERAL * toughCount)
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + this.roomName + ' - XGHO2 desired: ' + LAB_BOOST_MINERAL * toughCount + ', available: ' + Room.getStoredResourceAmount(spawn.room.name, 'XGHO2'));
            return null;
        }

        if (Room.getStoredResourceAmount(spawn.room.name, 'XKHO2') < LAB_BOOST_MINERAL * rangedAttackCount)
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + this.roomName + ' - XKHO2 desired: ' + LAB_BOOST_MINERAL * rangedAttackCount + ', available: ' + Room.getStoredResourceAmount(spawn.room.name, 'XKHO2'));
            return null;
        }

        if (Room.getStoredResourceAmount(spawn.room.name, 'XLHO2') < LAB_BOOST_MINERAL * healCount)
        {
            //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + this.roomName + ' - XLHO2 desired: ' + LAB_BOOST_MINERAL * healCount + ', available: ' + Room.getStoredResourceAmount(spawn.room.name, 'XLHO2'));
            return null;
        }
        ...
        if (roomMemory.controller && roomMemory.controller.o)
        {
            ...
            {
                //console.log('XXXXXXXXXXXXXXXX Job_Commando.getDesiredBoosts - ' + this.roomName + ' - XZH2O desired: ' + LAB_BOOST_MINERAL * workCount + ', available: ' + Room.getStoredResourceAmount(spawn.room.name, 'XZH2O'));
                return null;
            }
