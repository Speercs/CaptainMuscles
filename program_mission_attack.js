...
class Mission_Attack extends Mission_Creeps
{
    constructor (...args)
    {
        super(...args);
    ...
    {
        console.log('****************Mission_Attack.end - ' + this.data.room + ' - mission ended');
    ...
    refresh()
    {
        ...
        this.setMemory({ type: 'attack', room: this.data.room });
        ...
        this.targetRoom = Game.rooms[this.data.room];
        this.targetRoomMemory = Room.getMemory(this.data.room);

        this.flag = Game.flags['attack_' + this.data.room];

        if (this.targetRoom && !this.flag)
            this.targetRoom.createFlag(25, 25, 'attack_' + this.data.room);
    }
    ...
    creepAdded(creepName)
    {
        ...
        {
            console.log('attack.creepAdded - attempted to add ' + creepName + ' to mission again');
            return;
        }
        ...
        console.log('Mission_Attack.creepAdded - ' + creepName + ' added to squad');
    }
    ...
    {
        {
            ...
            if (creepIndex >= 0)
            {
                ...
                console.log('****************Mission_Attack.creepRemoved - ' + this.data.room + ' - ' + creepName + ' removed from mission');
            }
        ...
        if (squad)
        {
            ...
            console.log('****************Mission_Attack.creepRemoved - ' + creepName + ' removed from squad');
            ...
            if (squad.creeps.length <= 0)
            {
                ...
                console.log('Mission_Attack.creepRemoved - ' + this.data.room + ' - squad deleted');
            }
        }
    }
    ...
    updateDesiredCreeps(allCreeps, spawnedCreeps)
    {
        ...
        if (!this.targetRoomMemory /*|| !this.targetRoomMemory.hostiles || !this.targetRoomMemory.hostiles.tc*/ || creepsMoreThanHalfAlive.length >= this.maxTotal)
        {
    ...
    run()
    {
        ...
        if (this.targetRoom && this.targetRoom.controller && (!this.targetRoom.controller.owner || this.targetRoom.controller.my || this.targetRoom.controller.safeMode))
        {
            console.log('Mission_Attack.run - ' + this.data.room + ' - cannot attack room. Ending.');
            return this.suicide();
        }

        let creeps = this.getCreeps();

        if (!this.targetRoomMemory)
            return this.suicide();

        if (!this.targetRoomMemory.controller)// && !this.targetRoomMemory.hostiles)
            return this.suicide();

        if (creeps.length < this.minSquadSize && this.targetRoomMemory.controller && (!Memory.empire || !Memory.empire.warfare || !Memory.empire.warfare.targets || Memory.empire.warfare.targets.length <= 0 || !Memory.empire.warfare.targets.some(rn => rn == this.data.room)))
            return this.suicide();
        ...
        {
            if (!creep.spawning && !creep.hasTask({ n: 'task_attack', r: this.data.room }))
        ...
        let creepsForJob = Room.getJobCreeps(this.data.room, 'attack');
        ...
        // {
        //     console.log('Mission_Attack.run - do not want creeps');
        //     return this.suicide();
        // }
    ...
    {
        console.log('Mission_Attack.doSquadRenew - ' + this.data.room + ' RENEWING');
    ...
    {
        console.log('Mission_Attack.doSquadBoost - ' + this.data.room + ' BOOSTING');
    ...
    doSquad(squad)
    {
        ...
        if (squadFlee)
        {
            //console.log('Mission_Attack.doSquad - squad fleeing');
            squad.memory.flee = 1;
        ...
        let primaryTarget = this.selectPrimaryTarget(squad.leader.room.name, squad.leader);
        let range = 2;
        ...
        if (primaryTarget)
        {
            ...
            //console.log('Mission_Attack.doSquad - primary target at: ' + primaryTarget.pos);
    ...
    doSquadFlee(squad)
    {
        //console.log('Mission_Attack.doSquadFlee - squad fleeing');
    ...
    {
        console.log('Mission_Attack.doSquadDisband - ' + this.data.room + ' - disbanding squad');
    ...
    selectPrimaryTarget(roomName, creep)
    {
        ...
        (
            o =>
            {
                ...
                //console.log('Mission_Attack.selectPrimaryTarget - ' + creep.name + ' - looking for mapRoom flag');
