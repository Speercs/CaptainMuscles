...
class Task_Drain extends Task
{
    ...
    doTask(creep)
    {
        this.targetRoomName = this.memory.r;
        this.flag = Game.flags['drain_' + this.memory.r] || Game.flags['drain'];
        if (this.flag)
            this.targetRoomName = this.flag.pos.roomName;
        ...
        let targetRoomMemory = Room.getMemory(this.targetRoomName);
        if (!this.flag && (!targetRoomMemory || (targetRoomMemory.controller && targetRoomMemory.controller.sm && targetRoomMemory.controller.sm > Game.time)))
            return TASK_RESULT_COMPLETE;

        let inTargetRoom = (creep.room.name == this.targetRoomName);
        if (this.flag && inTargetRoom && creep.room.controller && creep.room.controller.safeMode)
            return TASK_RESULT_COMPLETE;
        ...
        if (primaryTarget)
        {
            ...
            Game.map.visual.line(creep.pos, primaryTarget.pos, {color: constants.PART_COLORS.WORK, lineStyle: 'dashed'});
            Game.map.visual.circle(primaryTarget.pos, {radius: 1, fill: constants.PART_COLORS.WORK, opacity: 1.0});
        }
        ...
        let creepFlag = Game.flags[creep.name];
        if (creepFlag)
        {
            this.gotoTarget(creepFlag, 0, { ignoreCreeps: false });
            canMove = false;
        }
        ...
        let targetRoomCenter = new RoomPosition(25, 25, this.targetRoomName);
        ...
        if (canMove && this.gotoRoom(this.targetRoomName, 0, { ignoreCreeps: false }))
            return TASK_RESULT_BREAK;

        if (!this.flag && !primaryTarget)
            return TASK_RESULT_COMPLETE;
    ...
    selectPrimaryTarget(creep)
    {
        ...
        let myFlag = Game.flags[creep.name] || this.flag;
        ...
        if (!inTargetRoom)
        {
            // let potentialTargets = creep.room.find(FIND_HOSTILE_CREEPS, { filter: cr => !cr.pos.nearEdge(1) && !cr.isInvader() && cr.killOnSight() && _.find(cr.body, p => p.type == ATTACK || p.type == RANGED_ATTACK) });
            // if (potentialTargets.length > 0)
        ...
        (
            o =>
            {
                ...
                if (o.structureType == STRUCTURE_POWER_BANK || o.structureType == STRUCTURE_CONTROLLER || o.structureType == STRUCTURE_WALL || o.structureType == STRUCTURE_EXTRACTOR)
                    return false;
        ...
        if (potentialTargets.length > 0)
            return _.min(potentialTargets, t => creep.pos.getRangeTo(t.pos));
    ...
    {
        let roomMemory = Room.getMemory(this.targetRoomName);
    ...
    selectTargetEnRouteToPrimaryTarget(creep, target)
    {
        ...
        {
            console.log('Task_Drain.selectTargetEnRouteToPrimaryTarget - ' + creep.pos + ' -> ' + target.pos + ' - could not find path');
            return null;
        }
    ...
    fleeDanger(creep)
    {
        ...
        let potentialThreats = creep.pos.lookForInRange(LOOK_CREEPS, 4, (pt => !pt.spawning && pt.attackInCombat() && (pt.healPower || pt.rangedAttackPower || (pt.attackPower && creep.pos.getRangeTo(pt) <= 2))));
        ...
        let enemyNetDamage = Math.max(1, totalEnemyDamage - totalAllyHealing);
        let allyNetDamage = Math.max(1, totalAllyDamage - totalEnemyHealing);
